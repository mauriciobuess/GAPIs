...
BEGIN
	...
	if EXISTS(SELECT 1
	            FROM [dbo].[DemandaCep] [C] WITH (NOLOCK)
			   WHERE [C].[isProcessado] != CAST(1 as BIT))
	BEGIN
		INSERT INTO [dbo].[ApiDemandaComunica]
				  ...
				  SELECT TOP 1
				         [Api].[idApi]              ,GETDATE()                              ,[Api].[nome]
				        ,[dbo].[fncConstroiEndPoint]([Api].[idApi], 'cepTi4@ll')
						,[dbo].[fncConstroiCorpo]([Api].[idApi], 'cepTi4@ll')
						,'A' + CAST(idAPi AS VARCHAR(03)) + '|cep|' + 'cepTi4@ll'
						,'API Busca CEP ViaCep'
						, CAST(0 AS BIT)			,NULL
						, NULL                      ,NULL                                   ,NULL
						, CAST(0 AS BIT)            ,NULL
				    FROM [dbo].[Api] WITH (NOLOCK) 
				   WHERE [Api].[idApi] = 1
				     ...
				     AND NOT EXISTS(SELECT 1
									 ...
									 WHERE [ADC].[isDemandaAtendida] != CAST(1 AS BIT) 
									   AND [ADC].[chaveExterna] = 'A' + CAST(idAPi AS VARCHAR(03)) + '|cep')
		...
		INSERT INTO [dbo].[ApiDemandaComunica]
				  ...
				  SELECT TOP 1
				         [Api].[idApi]              ,GETDATE()                              ,[Api].[nome]
				        ,[dbo].[fncConstroiEndPoint]([Api].[idApi], '05576805000100')
						,[dbo].[fncConstroiCorpo]([Api].[idApi], '05576805000100')
						,'A' + CAST(idAPi AS VARCHAR(03)) + '|cep|' + '05576805000100'
						,'API Busca CNPJ IBGE'
						, CAST(0 AS BIT)			,NULL
						, NULL                      ,NULL                                   ,NULL
						, CAST(0 AS BIT)            ,NULL
				   ...
				   WHERE [Api].[idApi] = 2
				     ...
				     AND NOT EXISTS(SELECT 1
				                      FROM [dbo].[ApiDemandaComunica] [ADC] WITH (NOLOCK)
									 WHERE [ADC].[isDemandaAtendida] != CAST(1 AS BIT) 
									   AND [ADC].[chaveExterna] = 'A' + CAST(idAPi AS VARCHAR(03)) + '|CNPJ')
	...
	SET @Msql		= N' SELECT [A].[idApiDemandaComunica] ,[A].[idApi]	              ,[A].[dataDemanda]   ,[A].[nomeApi]      
	                       ...
	                       FROM (SELECT [ADC].[idApiDemandaComunica]     ,[ADC].[idApi]	              ,[ADC].[dataDemanda]         ,[ADC].[nomeApi]       
						               ...
						               ,COALESCE([ADC].[endPoint], [dbo].[fncConstroiEndPoint]([API].[idApi], null)) [endPoint]
									   ,[API].[isConstroiCorpo]     
									   ,COALESCE([ADC].[corpo], [dbo].[fncConstroiCorpo]([API].[idApi], null)) [corpo]
